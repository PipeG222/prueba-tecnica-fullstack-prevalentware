    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 403) {
          ...
          alert(data?.error ?? "No puedes dejar la organización sin administradores.");
        } else {
      ...
      <div className="mx-auto max-w-6xl px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          <nav className="hidden md:flex items-center gap-4 text-sm text-slate-600">
            ...
            {isAdmin && <Link href="/users" className="hover:text-slate-900">Usuarios (ADMIN)</Link>}
            {isAdmin && <Link href="/reports" className="hover:text-slate-900">Reportes (ADMIN)</Link>}
          </nav>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/movements" className="px-3 py-2 rounded bg-emerald-600 text-white text-sm">
              Panel
            </Link>
            <button onClick={signOut} className="px-3 py-2 rounded bg-slate-800 text-white text-sm">
              Cerrar sesión
            </button>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            <button onClick={signIn} className="px-3 py-2 rounded bg-blue-600 text-white text-sm">
              Iniciar sesión
            </button>
            <button onClick={signIn} className="px-3 py-2 rounded border border-blue-600 text-blue-600 text-sm">
              Registrarse
            </button>
          </div>
        )}
