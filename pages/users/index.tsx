...
export default function UsersPage() {
  ...
  const current = useMemo(() => rows.find((r) => r.id === editingId) ?? null, [rows, editingId]);
  const [form, setForm] = useState<{ name: string; phone: string; role: Role }>({ name: "", phone: "", role: "USER" });
  ...
  async function toggleRole(u: UserRow) {
    try {
      ...
      const res = await authFetch(`/api/users/${u.id}/role`, { method: "POST" });
      const data = await asJson(res);
      if (!res.ok) throw new Error(data?.error ?? "No se pudo cambiar el rol");
      setRows((prev) => prev.map((r) => (r.id === u.id ? { ...r, role: data.role as Role } : r)));
    } catch (e: any) {
      alert(e.message || String(e));
    }
  }
        ...
        {editingId && current && (
          <section className="rounded border p-4 max-w-xl space-y-3">
            ...
            <p className="text-xs text-slate-500">
              * Si cambias un ADMIN a USER y es el último admin, el servidor lo bloqueará.
            </p>
          </section>
        )}
      </main>
